...
| Status | B-ID | Item | Agent label | Pri | Pts |
|--------|------|------|-------------|-----|-----|
| 🟦 | B-194 | Heavy compute on runner-LOCAL SQLite (download→compute DD_FORCE_SQLITE→upload). **DESIGNED** (infra-review, low-risk, mostly reuse) — design: `b194-local-compute-design.md`. **Phase 1 DONE + PASS** (Rupert ran `download_from_postgres.py` 2026-06-25 — all 12 tables parity OK, incl. prices 784,812; bug fixed = set DD_FORCE_SQLITE before db.migrate so it builds a sqlite schema while DD_DATABASE_URL drives the psycopg source). **Phase 3a CODED**: `.github/workflows/b194-test.yml` — read-only speed test (download → run full pipeline DD_FORCE_SQLITE → NO upload). Next: Rupert push + trigger; confirm pipeline reaches build_dashboard without timing out. Then add upload (Phase 3b) + promote. Reuses existing secrets; no new token. | `agent:general-purpose` | P1 | 8 |
