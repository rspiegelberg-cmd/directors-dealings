...
#     28/147 rows use a RENORMALIZED 3-key weight set (who=0.36585, buy_size=0.36585,
...
#     5-factor score, or the simulation silently misrepresents ~19% of the cohort.
# ---------------------------------------------------------------------------
...
#    they sum to 1.0, rather than silently letting f4's weight vanish from
#    the denominator (which would be a DIFFERENT, undocumented behaviour change).
#    who/buy_size/company_size keep their *relative* proportions to each other
#    and to sector; only earnings_timing's 0.144 share is redistributed.
# ---------------------------------------------------------------------------
...
# Structural max possible sector contribution
...
print(f"Structural min sector contribution to score (subscore=0.0): {0.0:.2f} points")
print()

# If ceiling hits under new design exist, check whether their OTHER 4 factors
# alone (without sector) already summed to >= 0.80 (i.e. the 80% remaining
# weight alone could reach 100 given clamp), to see if sector is "causing" any
# residual ceiling hit or whether it's organic.
...
if not ceiling_new:
    ...
    for r in ceiling_new:
        ...
        print(f"  {r['ticker']:6s} {r['director']:30s} core-only-sum={core_only:.4f} has_earnings={r['has_earnings']} (need >=0.80 alone to hit ceiling w/o any sector help)")
print()

# ---------------------------------------------------------------------------
# 8. Top 20-30 by OLD rank: old vs new
# ---------------------------------------------------------------------------
...
# Also compare vs Part 2's additive-nudge design if cheap to reconstruct.
# Part 2 formula: score_new2 = clamp(100*ws_old4 + tanh(weighted_net/8)*6, 0, 100)
# We don't have the exact weighted_net(halflife=14) per row's sector re-derivable
# here without the daily transaction feed (out of scope/cost for this pass) --
# skip unless cheap. We DO have Part 2's per-sector adj_pts for the sectors
# reported in Part 2's simulation table for a rough proxy comparison instead.
PART2_ADJ_PTS = {
    "Financials": 4.45,
    "Industrials": 4.47,
    ...
    # sectors not explicitly in Part 2's printed table: leave unmapped (skip in proxy)
}
...
if proxy_rows:
    for r in proxy_rows:
        ws_old4 = (OLD_WEIGHTS["who"]*r["f1"] + OLD_WEIGHTS["buy_size"]*r["f2"] +
                   OLD_WEIGHTS["company_size"]*r["f3"] + OLD_WEIGHTS["earnings_timing"]*r["f4"])
        r["score_part2_proxy"] = max(0.0, min(100.0, 100*ws_old4 + PART2_ADJ_PTS[r["sector"]]))
    sorted_p2 = sorted(proxy_rows, key=lambda r: -r["score_part2_proxy"])
    for i, r in enumerate(sorted_p2, start=1):
        r["rank_part2_proxy"] = i
    ranks_new_sub = [r["rank_new"] for r in proxy_rows]
    # re-rank within subset using original 'rank_old' position among subset for fair proxy comparison
    ranks_old_sub_pos = sorted(range(len(proxy_rows)), key=lambda i: proxy_rows[i]["rank_old"])
    # Simplify: just report rho between full new ranks and part2 proxy full-set positions using rank_old as tiebreak baseline
...
print(f"  Part 3 sector max swing (subscore 0->1): {1.0*0.20*100:.1f} points (full range, {0.0*0.20*100:.1f} to {1.0*0.20*100:.1f})")
print(f"  Part 2 additive nudge cap: +/-6.0 points")
print(f"  Ratio: Part 3 max range is {(1.0*0.20*100)/6.0:.2f}x wider than Part 2's nudge cap on the upside alone")
print(f"  (Part 2 range was -6 to +6 = 12pt span; Part 3 range is 0 to +20 = 20pt span -> {20/12:.2f}x wider span)")
