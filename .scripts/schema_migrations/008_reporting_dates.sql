-- Migration 008: create reporting_dates table (B-096).
-- Idempotent: CREATE TABLE IF NOT EXISTS.
--
-- Stores known and upcoming financial results dates per ticker.
-- Source: Yahoo Finance calendarEvents module via backfill_reporting_dates.py.
--
-- report_type:
...
--   EARNINGS       -- generic earnings date from Yahoo (type not yet resolved)
--
-- 60-day badge logic (B-096): when a transaction date falls within 60 calendar
-- days BEFORE a reporting date for the same ticker, render a yellow warning
-- badge. This is wider than the 30-day MAR legal closed period (Rupert 2026-06-03).
--
